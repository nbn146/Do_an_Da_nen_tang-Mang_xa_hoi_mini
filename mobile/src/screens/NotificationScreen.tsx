...
const getNotificationIcon = (type: string) => {
  switch (type) {
    case "like":
      return <Heart color="#ef4444" size={16} fill="#ef4444" />;
    case "comment":
      return <MessageCircle color="#3b82f6" size={16} />;
    case "follow":
      return <UserPlus color="#a855f7" size={16} />;
    case "mention":
      return <AtSign color="#22c55e" size={16} />;
    case "share":
      return <Share2 color="#f97316" size={16} />;
    default:
      return <Heart color="#6b7280" size={16} />;
  }
};
...
const getNotificationText = (
  ...
  switch (type) {
    case "like":
      return t("đã thích bài viết của bạn", "liked your post");
    case "comment":
      return t("đã bình luận bài viết của bạn", "commented on your post");
    case "follow":
      return t("đã bắt đầu theo dõi bạn", "started following you");
    case "mention":
      return t("đã nhắc đến bạn", "mentioned you");
    case "share":
      return t("đã chia sẻ bài viết của bạn", "shared your post");
    default:
      return t("đã tương tác với bạn", "interacted with you");
  }
};
...
export default function NotificationsScreen() {
  ...
  const renderItem = useCallback(
    ({ item }: { item: INotification }) => {
      const sender = (
        typeof item.sender_id === "object" ? item.sender_id : null
      ) as IUser | null;
      const senderName =
        sender?.display_name || sender?.username || t("Ai đó", "Someone");
      const senderAvatar =
        sender?.avatar_url ||
      ...
      return (
        <Pressable
          ...
          style={[styles.notifRow, !item.is_read ? styles.notifUnread : null]}
        >
